...
The subsequent report offers insights into the distribution of inline barcodes 
within a conventional Illumina plate consisting of 96 wells. The pipeline 
produces a variety of standardized plots aimed at revealing prevalent 
contamination patterns within the dataset. Instructions on how to read the plots
are provided at the top of each plot.
...
data_dir = "/Users/jar4142/Desktop/Report/Merge/"
out_dir = "/Users/jar4142/Desktop/Report/"
path_to_flist = "/Users/jar4142/Desktop/Test_Reduced_Datasets/Keltonlyci_test/barcodes_matches.csv"
minreads_threshold = 1000
contamination_threshold = 0.5
missing_files = "/Users/jar4142/Desktop/Results/missing_files.tsv"
...
#Remove the reads with No_barcodes. It's impossible to determine if the failure 
#to detect barcodes is due to their absence associated with a PCR error or 
#because the read belongs to a non-barcoded target.

df = df[which(df$match_status != "No_barcodes"),]
...
As pointed out before, some file, though present in the fastq repository, may have been removed by the pipeline due to low quality or unusable data. These files are listed below and are missing from subsequent pots an analysis.
...
# The below threshold flag signals if no barcode was dominant in the well per the
# threshold provided in contamination_threshold. Wrong barcode flag signals if
# the most common barcode in the well corresponds or not to the expected barcode
# for said well.
...
m_sample_status_p_melted_reshaped$Flagged_well = m_sample_status_p_melted_reshaped$Productivity_Flag |
  m_sample_status_p_melted_reshaped$Wrong_Barcode_Comb_Flag | m_sample_status_p_melted_reshaped$Below_Threshold_Flag
...
#  h = sample(1:12, 4, replace = TRUE),
#  v = sample(1:8, 4, replace = TRUE),
#  h_target = sample(1:12, 4, replace = TRUE),
#  v_target = sample(1:8, 4, replace = TRUE)
#)
...
# Add each segment in df_arrows to the plot 'p' using a for loop
# for (i in 1:nrow(df_arrows)) {
#   p = p + geom_segment(aes(x = df_arrows$h[i], y = df_arrows$v[i], 
#                             xend = df_arrows$h_target[i], yend = df_arrows$v_target[i]), 
#                         arrow = arrow(length = unit(0.2, "inches")), 
#                         color = "blue", size = 3, lineend = "round", linejoin = "round")
# }

for (i in 1:nrow(df_arrows)) {
  p = p + annotate("segment", x = df_arrows$h[i], y = df_arrows$v[i], 
                   xend = df_arrows$h_target[i], yend = df_arrows$v_target[i],
                   color = "blue", size = 3,
                   arrow = arrow(ends = "last", angle= 45, type = "closed",
                                 length = unit(.3,"cm")))
}
...
Well Flagged for both contamination, low productivity and being below threshold
